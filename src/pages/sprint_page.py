...
class SprintPage(BasePage):
    ...
    sprint_project =     (By.XPATH, "//div[contains(@class, 'q-dialog')]//label[.//div[contains(text(), 'Project')]]//div[contains(@class, 'q-field__control')]")
    # 스프린트 프로젝트 선택 필드
 
    sprint_project_select = (By.XPATH, "//div[@role='option']") # 스프린트 프로젝트 선택 옵션
    sprint_status = (By.XPATH, "//div[contains(@class, 'q-dialog')]//input[@aria-label='Status']/ancestor::label") # 스프린트 상태 선택 필드
    ...
    def create_sprint(self, sprint_name, project_name=None):
        ...
        try:
            ...
            self.wait.until(EC.visibility_of_element_located(self.sprint_project_select)) # 옵션창이 뜰 때까지 대기
            
            if project_name:
                print(f"🔎 프로젝트 찾는 중: {project_name}")
                # 🌟 [핵심] 특정 텍스트를 포함하는 옵션을 찾는 동적 XPath
                specific_option_xpath = f"//div[@role='option'][contains(., '{project_name}')]"
                ...
                try:
                    ...
                    print(f"⚠️ '{project_name}' 옵션을 찾지 못했습니다. 첫 번째 옵션을 대신 선택합니다.")

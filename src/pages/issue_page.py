...
class issuePage(BasePage): 
    ...
    issue_project_select = (By.XPATH, "//div[@role='listbox']") # 이슈 프로젝트 선택 옵션
    issue_type = (By.XPATH, "//div[contains(@class, 'q-field__control') and .//div[contains(., 'Type *')]]") # 이슈 타입 선택 필드
    issue_type_select = (By.XPATH, "//div[@role='option']//span[contains(text(), 'Bug')]") # 이슈 타입 선택 옵션
    ...
    def create_issue(self, issue_name,project_name=None):
        ...
        try:
            ...
            self.wait.until(EC.visibility_of_element_located(self.issue_project_select)) # 옵션창이 뜰 때까지 대기
